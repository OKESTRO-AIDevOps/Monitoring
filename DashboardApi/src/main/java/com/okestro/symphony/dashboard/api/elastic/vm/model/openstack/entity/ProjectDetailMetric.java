...
public class ProjectDetailMetric implements Serializable {

    @Id
    private String id;
//    @Field(name = "@timestamp", type = FieldType.Date, store = true, format = DateFormat.custom, pattern = "YYYY-MM-dd'T'HH:mm:ss.SSSX")            // store 옵션은 application on 할때 최초에 생성.
    @Field(name = "@timestamp")            // store 옵션은 application on 할때 최초에 생성.
    ...
    public static class ProjectDetailInfo {
        ...
        private ProjectDetailInfo.ProjectQuotaSet projectQuotaSet;
        @Field(name = "project_usage")
        private ProjectDetailInfo.ProjectUsages projectUsage;
        @Field(name = "project_minus")
        private ProjectDetailInfo.ProjectUsages projectMinus;
